...
router.get("/read", (req, res) => {
  ...
  let query = "SELECT * FROM inquiry_lead_activity WHERE 1=1";
  const params = [];
...
router.post("/insert", (req, res) => {
  ...
  const query = "INSERT INTO inquiry_lead_activity (name) VALUES (?)";
...
router.put("/update/:id", (req, res) => {
  ...
  const query = "UPDATE inquiry_lead_activity SET name = ? WHERE id = ?";
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE inquiry_lead_activity SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM inquiry_lead_activity");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM inquiry_lead_activity ORDER BY id ASC LIMIT ? OFFSET ?`;
