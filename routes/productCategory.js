...
router.post("/insert", (req, res) => {
  ...
  const query = "INSERT INTO product_category (name, status) VALUES (?, ?)";
...
router.put("/update/:id", (req, res) => {
  ...
  const query = "UPDATE product_category SET name = ?, status = ? WHERE id = ?";
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE product_category SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM product_category");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM product_category ORDER BY id ASC LIMIT ? OFFSET ?`;
...
router.get("/product-category", (req, res) => {
  const sql = "SELECT id, name FROM product_category ORDER BY id ASC";
