...
router.get("/read", (req, res) => {
  ...
  let query = "SELECT * FROM quote_status WHERE 1=1";
  const params = [];
...
router.post("/insert", (req, res) => {
  ...
  const query = "INSERT INTO quote_status (name, status) VALUES (?, ?)";
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE quote_status SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM quote_status");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM quote_status ORDER BY id ASC LIMIT ? OFFSET ?`;
...
router.get("/quote", (req, res) => {
  ...
  let sql = "SELECT id,name FROM quote_status";
  const values = [];
