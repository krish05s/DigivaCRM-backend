...
router.get("/read", async (req, res) => {
  try {
    ...
    let query = "SELECT * FROM ticket_type WHERE 1=1";
    const params = [];
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE ticket_type SET status = ? WHERE id = ?";
...
router.put("/default/:id", (req, res) => {
  ...
  const query = "UPDATE ticket_type SET `default` = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM ticket_type");
    ...
    const query = `SELECT id, ${columns.map(c => `\`${c}\``).join(", ")} FROM ticket_type ORDER BY id ASC LIMIT ? OFFSET ? `;
