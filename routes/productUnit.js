...
router.post("/insert", (req, res) => {
  ...
  const query = "INSERT INTO product_unit (name, status) VALUES (?, ?)";
...
router.put("/update/:id", (req, res) => {
  ...
  const query = "UPDATE product_unit SET name = ?, status = ? WHERE id = ?";
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE product_unit SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM product_unit");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM product_unit ORDER BY id ASC LIMIT ? OFFSET ?`;
...
router.get("/product-unit", (req, res) => {
  const sql = "SELECT id,name FROM product_unit ORDER BY name ASC";
