...
router.get("/read", (req, res) => {
  ...
  let query = "SELECT * FROM expense_sub_category WHERE 1=1";
  const params = [];
...
router.post("/insert", (req, res) => {
  ...
  const query = "INSERT INTO expense_sub_category (name, status) VALUES (?, ?)";
...
router.put("/update/:id", (req, res) => {
  ...
  const query = "UPDATE expense_sub_category SET name = ?, status = ? WHERE id = ?";
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE expense_sub_category SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM expense_sub_category");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM expense_sub_category ORDER BY id ASC LIMIT ? OFFSET ?`;
