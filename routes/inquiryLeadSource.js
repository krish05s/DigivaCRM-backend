...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE inquiry_lead_source SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM inquiry_lead_source");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM inquiry_lead_source ORDER BY id ASC LIMIT ? OFFSET ?`;
...
router.get("/lead-source", (req, res) => {
  ...
  let sql = "SELECT name FROM inquiry_lead_source";
  const values = [];
