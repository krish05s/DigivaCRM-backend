...
router.get("/read", (req, res) => {
  ...
  let query = "SELECT * FROM role_master WHERE 1=1";
  const params = [];
...
router.post("/insert", (req, res) => {
  ...
  const query = "INSERT INTO role_master (name, status) VALUES (?, ?)";
...
router.put("/update/:id", (req, res) => {
  ...
  const query = "UPDATE role_master SET name = ?, status = ? WHERE id = ?";
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE role_master SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM role_master");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM role_master ORDER BY id ASC LIMIT ? OFFSET ?`;
...
router.get("/role-name", (req, res) => {
  const sql = "SELECT id, name FROM role_master ORDER BY name ASC";
