...
router.post("/insert", (req, res) => {
  ...
  const query = "INSERT INTO task_status (name, status) VALUES (?, ?)";
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE task_status SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM task_status");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM task_status ORDER BY id ASC LIMIT ? OFFSET ?`;
