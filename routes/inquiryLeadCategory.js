...
router.put("/update/:id", (req, res) => {
  ...
  const query = "UPDATE inquiry_lead_category SET name = ? WHERE id = ?";
...
router.put("/status/:id", (req, res) => {
  ...
  const query = "UPDATE inquiry_lead_category SET status = ? WHERE id = ?";
...
router.get("/get-column-scroll", async (req, res) => {
  try {
    ...
    // Get total records
    const [countRows] = await db.promise().query("SELECT COUNT(*) AS total FROM inquiry_lead_category");
    ...
    //  Fixed query
    const query = `SELECT \`${column}\` FROM inquiry_lead_category ORDER BY id ASC LIMIT ? OFFSET ?`;
...
router.get("/lead-category", (req, res) => {
  const { status } = req.query;

  let sql = "SELECT name FROM inquiry_lead_category";
  const values = [];
